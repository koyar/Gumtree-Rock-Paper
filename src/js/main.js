...
RockPaper.prototype.createPlayers = function(player) {
  ...
  if(this.player1.playerType === 'cpu' && this.player2.playerType === 'cpu') {
    ...
    setInterval(function() { 
      if(that.player1.score < that.bestOfScore && that.player1.score < that.bestOfScore) {
          i++;
         that.playGame('cpu');
      }
    }, 2000);
...
CPUPlayer.prototype.play = function () {
  ...
  for (prop in this.moves) {
    if (this.moves.hasOwnProperty.call(this.moves, prop)) {
      result.push(prop);
    }
  }

  moves = Math.floor(Math.random() * result.length);
  this.moveMade = result[moves];
